...
'''
    ...
    async def _create_pull_request_with_issue_linking(self, story_id: str, story_data: Dict[str, Any], 
                                                    implementation_results: List[Dict[str, Any]]) -> str:
        """
        Create pull request with automatic issue linking for story closure.
        """
        try:
            print(f"🔄 Creating linked pull request for {story_id}")
            
            if not self.git_available:
                return "⚠️  Git not available, PR creation skipped"
            
            # Get story issue number and parent feature number
            story_issue_number = story_data.get("github_issue_number")
            parent_feature_number = story_data.get("parent_feature_number")
            
            # Create PR title with issue references
            ...
            # Create PR body with issue linking keywords
            pr_body = f"""## 🤖 AI-Generated Feature Implementation

    **Story**: #{story_issue_number} - {story_title}
    **Parent Feature**: #{parent_feature_number}
    **Generated by**: DigiNativa AI Utvecklare (Claude-3.5-Sonnet)
    ...
    """
            ...
            # Add GitHub issue closing keywords
            pr_body += f"""
    ### 🔗 Linked Issues
    - Closes #{story_issue_number}
    - Relates to #{parent_feature_number}
    ...
    **Estimated review time**: 15-30 minutes
    ...
    - ✅ Story #{story_issue_number} will automatically close
    - 📊 Parent feature #{parent_feature_number} progress will update
    - 🎉 Feature will close when all related stories are complete
    ...
    *This PR was automatically created by the DigiNativa AI development team*
    """
            ...
            if "✅" in pr_result:
                print(f"✅ Pull request created with issue linking")
                
                # Extract PR number from result if available
                import re
                pr_number_match = re.search(r'#(\d+)', pr_result)
                if pr_number_match:
                    pr_number = pr_number_match.group(1)
                    
                    # Link PR to development field of issues
                    await self._link_pr_to_issues(pr_number, story_issue_number, parent_feature_number)
            
            return pr_result
            
        except Exception as e:
            return f"❌ PR creation with linking failed: {str(e)}"

    async def _link_pr_to_issues(self, pr_number: str, story_issue: str, parent_feature: str):
        """
        Link PR to GitHub issues using development field.
        
        This enables automatic closure when PR is merged.
        """
        try:
            # This would use GitHub's GraphQL API to link PR to issues
            # For now, we rely on the "Closes #X" keywords in PR body
            print(f"🔗 PR #{pr_number} linked to close issues #{story_issue} and relate to #{parent_feature}")
            
        except Exception as e:
            print(f"⚠️  Could not link PR to issues: {e}")
